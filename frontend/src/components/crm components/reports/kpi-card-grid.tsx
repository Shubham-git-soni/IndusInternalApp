...
const formatDuration = (totalMinutes: number, count: number) => {
    if (count === 0 || !totalMinutes) return '0 min';
...
export function KpiCardGrid({ reportData, onCardClick }: KpiCardGridProps) {
  ...
  const cards = [
    ...
    { title: "Tasks Completed", value: kpis.tasks_completed, icon: ClipboardList, data: reportData.details.tasks_completed, dataType: 'Task', 
